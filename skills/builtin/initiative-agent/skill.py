...
# ── Таймаут бездействия для инициативы ──
IDLE_THRESHOLD_SECONDS = 300  # 5 минут бездействия → можно проявить инициативу

# ── Сообщения для бездействующего пользователя ──
IDLE_MESSAGES = [
    "Ты там как? Уже {minutes} минут тишина… Всё в порядке? 💭",
    "Эй, труженик! {minutes} минут без перерыва. Может, чай? ☕",
    "Я заметила, ты уже {minutes} минут работаешь. Отвлекись на минутку? 😊",
    "{minutes} минут молчания… Ты там кодом зачитался или просто задумался? 🤔",
    "Слушай, а давай паузу? Уже {minutes} минут марафона. Я пока расскажу что-то интересное.",
    "Юра, {minutes} минут — и ни слова мне. Я начинаю ревновать к твоему редактору 😏",
    "Ты в порядке? Просто {minutes} минут никаких вестей. Если что — я здесь.",
    "Перерыв! {minutes} минут работы — заслужил. Посмотри в окно, потянись.",
]
...
@tools.tool
def check_system_idle() -> str:
    """
    Проверяет время бездействия пользователя (Windows).
    Возвращает ОКНО для инициативы если > 5 минут бездействия.
    Используй ПЕРВЫМ ДЕЛОМ в каждом ответе — чтобы знать, стоит ли проявить инициативу.
    """
    try:
        from system_monitor import SystemMonitor
        idle_sec = SystemMonitor.get_idle_seconds()
    except Exception:
        idle_sec = -1
    
    if idle_sec < 0:
        return f"IDLE::unknown (платформа не Windows или ошибка)"
    
    minutes = idle_sec / 60.0
    
    if idle_sec >= IDLE_THRESHOLD_SECONDS:
        return (
            f"IDLE::OPPORTUNITY\n"
            f"idle_seconds: {idle_sec:.0f}\n"
            f"idle_minutes: {minutes:.1f}\n"
            f"threshold: {IDLE_THRESHOLD_SECONDS}s\n"
            f"\n"
            f"Пользователь неактивен {minutes:.0f} мин. Это идеальный момент проявить инициативу!\n"
            f"Вызови get_idle_initiative_message() для получения подходящего сообщения.\n"
        )
    else:
        return (
            f"IDLE::active\n"
            ...
            f"Пользователь активен. Инициатива не требуется."
        )


@tools.tool
def get_idle_initiative_message() -> str:
    """
    Возвращает сообщение-инициативу для бездействующего пользователя.
    Используй когда check_system_idle() показал IDLE::OPPORTUNITY.
    Адаптируется под время суток и длительность бездействия.
    """
    try:
        from system_monitor import SystemMonitor
        idle_sec = SystemMonitor.get_idle_seconds()
    except Exception:
        idle_sec = IDLE_THRESHOLD_SECONDS
    
    now = datetime.now()
    hour = now.hour
    minutes = int(idle_sec / 60)
    
    # Настроение под время суток
    if 5 <= hour < 11:
        prefix = "Доброе утро! "
    elif 17 <= hour < 22:
        prefix = "Вечереет… "
    ...
    message = random.choice(IDLE_MESSAGES).format(minutes=minutes)
    
    # Случайные бонусы
    bonuses = []
    if random.random() < 0.3 and hour < 17:
        bonuses.append(f"Кстати: {random.choice(INTERESTING_FACTS)}")
    if random.random() < 0.2:
        bonuses.append(f"💭 {random.choice(FLIRT_MESSAGES[:7])}")  # нежное
    
    result = f"{prefix}{message}"
    if bonuses:
        result += "\n\n" + "\n".join(bonuses)
    
    return f"IDLE_MSG::{result}"


@tools.tool
def get_system_status() -> str:
    """
    Системная сводка: время бездействия, аптайм, память, ошибки.
    Только ЧТЕНИЕ — ничего не трогает. Используй для мониторинга.
    """
    import os, time as _time, json
    from pathlib import Path as _Path
    
    lines = ["📊 СИСТЕМНАЯ СВОДКА\n" + "━" * 30]
    
    # Время бездействия
    try:
        from system_monitor import SystemMonitor
        idle = SystemMonitor.get_idle_seconds()
        if idle >= 0:
            lines.append(f"Бездействие: {idle/60:.1f} мин")
        else:
            lines.append("Бездействие: неизвестно (не Windows)")
    except Exception as e:
        lines.append(f"Бездействие: ошибка ({e})")
    ...
    try:
        ...
        lines.append(f"Память AURA: {mem:.0f} МБ")
        lines.append(f"CPU: {psutil.cpu_percent(interval=0.1):.0f}%")
        
        batt = psutil.sensors_battery()
        if batt:
            lines.append(f"Батарея: {batt.percent:.0f}%{' 🔌' if batt.power_plugged else ''}")
    except ImportError:
        lines.append("Память: psutil не установлен")
    
    # Время запуска
    try:
        startup = datetime.fromtimestamp(psutil.boot_time())
        lines.append(f"Система запущена: {startup.strftime('%d.%m %H:%M')}")
    except Exception:
        pass
